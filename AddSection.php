<?php
session_start();
?>

<link rel="stylesheet" href="stylesheet.css">
<!doctype html>
<html>

<head>
    <ul>
    <li><img src = "UpdatedLogo.png" style="width:136;height:131;padding:10px;"></li>
    <!-- <li><a class="active" href="#home">Home</a></li> -->
    <li><div class="heading">The University of Western Long Island</div></li>
    </ul>
    <title>Add Section</title>
</head>

<body>
    <h1>Add Section</h1>
    <a href="BackToHomepage.php">Back to Homepage</a><br>
    <a href="ViewMasterSchedule.php">View Master Schedule</a><br> 

    <?php
        $servername = "localhost";
        $username = "root";
        $password = "";
        $dbname = "RegistrationSystem";

        // Create connection
        $conn = new mysqli($servername, $username, $password, $dbname);
        // Check connection
        if (!$conn) {
            die("Connection failed: " . mysqli_connect_error());
        }

        if($_SESSION["type"] != "Admin")
            {header('Location: UserHomepage.php');}

        $currentSeason = NULL;
        $date = getdate();
        $currentYear = $date["year"];
        $today = new DateTime();

        $winterStart = new DateTime('January 01');
        $winterEnd = new DateTime('January 18');
        $springStart = new DateTime('January 19');
        $springEnd = new DateTime('May 19');
        $summerStart = new DateTime('May 20');
        $summerEnd = new DateTime('August 31');
        $fallStart = new DateTime('September 01');
        $fallEnd = new DateTime('December 31');

        if(($today >= $winterStart) && ($today < $winterEnd))
            $currentSeason = "Winter";
        else if(($today >= $springStart) && ($today < $springEnd))
            $currentSeason = "Spring";
        else if(($today >= $summerStart) && ($today < $summerEnd))
            $currentSeason = "Summer";
        else if(($today >= $fallStart) && ($today < $fallEnd))
            $currentSeason = "Fall";

        $sql = "SELECT semester_ID FROM semesteryear WHERE Year = '" . $currentYear . "'" . " AND Season = '" . $currentSeason . "'";
        $result = $conn->query($sql);
        $row = $result->fetch_assoc();

        $currentSemesterID = $row["semester_ID"];

        $sql = "SELECT department_Name FROM department WHERE hidden = 0";
        $departmentList = $conn->query($sql);

        $sql = "SELECT course_ID, course_Name, department_Name FROM course WHERE hidden = 0";
        $courseList = $conn->query($sql);

        $sql = "SELECT * FROM semesteryear WHERE semester_ID >= '" . $currentSemesterID . "'";
        $semesterList = $conn->query($sql);

        $sql = "SELECT ID, first_Name, last_Name, department_Name FROM faculty WHERE hidden = 0";
        $facultyList = $conn->query($sql);

        $sql = "SELECT * FROM days";
        $daysList = $conn->query($sql);

        $sql = "SELECT * FROM period";
        $periodList = $conn->query($sql);

        $sql = "SELECT * FROM room";
        $roomList = $conn->query($sql);
    ?>

    <form method="post" action="AddSectionPost.php">
        <p>
            <label>
                Department:
                <select name="department">
                    <?php while($row = mysqli_fetch_array($departmentList))
                    {echo "<option>" . $row["department_Name"] . "</option>";} ?>
                </select>
            </label>
        </p>
        <p>
            <label>
                Course:
                <select name="courseID">
                    <?php while($row = mysqli_fetch_array($courseList))
                    {echo "<option value = '" . $row["course_ID"] . "'>" . $row["department_Name"] . " - " . $row["course_Name"] . "</option>";} ?>
                </select>
            </label>
        </p>
        <p>
            <label>
                Section #:
                <input name="sectionNum" type="number" size="1"  value ="1" min="0" max="9" required>
            </label>
        </p>
        
        <p>
            <label>
                Instructor:
                <select name="facultyID">
                    <?php while($row = mysqli_fetch_array($facultyList))
                    {echo "<option value = '" . $row["ID"] . "'>"
                     . $row["department_Name"] . " - " . $row["first_Name"] . " " . $row["last_Name"] . "</option>";
                    } ?>
                </select>
            </label>
        </p>
        <p>
            <label>
                Capacity:
                <input name="capacity" type="number" size="2" value="20" min="0" max="40" required>
            </label>
        </p>
        <p>
            <label>
                Days:
                <select name="dayID">
                    <?php while($row = mysqli_fetch_array($daysList))
                    {echo "<option value = '" . $row["day_ID"] . "'>" . $row["Days"] . "</option>";} ?>
                </select>
            </label>
        </p>
        <p>
            <label>
                Period:
                <select name="periodID">
                    <?php while($row = mysqli_fetch_array($periodList))
                    {echo "<option value = '" . $row["period_ID"] . "'>" . $row["period_Time"] . "</option>";} ?>
                </select>
            </label>
        </p>
        <p>
            <label>
                Semester:
                <select name="semesterID">
                    <?php while($row = mysqli_fetch_array($semesterList))
                    {echo "<option value = '" . $row["semester_ID"] . "'>" . $row["Season"] . " " . $row["Year"] . "</option>";} ?>
                </select>
            </label>
        </p>
        <p>
            <label>
                Building/Room:
                <select name="roomID">
                    <?php while($row = mysqli_fetch_array($roomList))
                    {
                        $sql = "SELECT building_Name FROM building WHERE building_Code = '" . $row["building_Code"] . "'";
                        $result = $conn->query($sql);
                        $row2 = $result->fetch_assoc();

                        echo "<option value = '" . $row["room_ID"] . "'>" . $row2["building_Name"] . ": " . $row["room_Number"] . "</option>";
                    } ?>
                </select>
            </label>
        </p>

        <p>
            <input type="submit" value="Submit">
            <input type="reset" value="Clear">
        </p> 
    </form>

    <p>
        <?php 
        if($_SESSION["Error"] != "") echo $_SESSION["Error"]; 
        $_SESSION["Error"] = "";
        ?>
    </p>

</body>
</html>